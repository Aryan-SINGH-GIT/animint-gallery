...
myread <- function(csv, repo.col){
  if(!file.exists(csv) || file.size(csv) == 0) return(data.table())
  nc::capture_first_df(
...
repos_dt <- data.table::fread("repos.txt", col.names="viz_owner_repo", header=FALSE)

meta_dt <- myread("meta.csv")[, let(
...
error_dt <- myread("error.csv", "viz_owner_repo")
```
